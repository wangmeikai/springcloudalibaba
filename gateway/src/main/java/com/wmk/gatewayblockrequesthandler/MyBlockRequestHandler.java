 ...
 * @TIME: 17:46
 **/
@Component    // gateway整合sentinel自定义限流降级处理
public class MyBlockRequestHandler extends DefaultBlockRequestHandler {
    ...
    private MyBlockRequestHandler.ErrorResult buildErrorResult(Throwable ex) {
        if(ex instanceof ParamFlowException) {
            return  new MyBlockRequestHandler.ErrorResult(HttpStatus.TOO_MANY_REQUESTS.value(),ex.toString());
        }else if (ex instanceof DegradeException) {
            return new MyBlockRequestHandler.ErrorResult(HttpStatus.TOO_MANY_REQUESTS.value(),ex.toString());
        }else{
            return new MyBlockRequestHandler.ErrorResult(HttpStatus.BAD_GATEWAY.value(),ex.toString());
        }
    }
